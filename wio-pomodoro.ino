...
void erase_clock(int x, int y) {
    tft.fillRect(x, y, CLOCK_LEN, FONT_SIZE + 2, TFT_BLACK);
}

void update(int state_changed, int blink) {
    int image_x = SCREEN_W / 2 - CLOCK_LEN / 2 - IMAGE_SIZE / 2;
    int image_y = SCREEN_H / 2 - IMAGE_SIZE / 2;
    int clock_x = image_x + IMAGE_SIZE + FONT_SIZE / 2;
...
void loop() {
    ...
    if (running) {
        run(state);
    } else if (state != STOPPED && millis() - last_blink >= BLINK_MS) {
        update(0, 1);
        last_blink = millis();
    }
}
